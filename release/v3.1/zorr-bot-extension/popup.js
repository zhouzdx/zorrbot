...
function saveSettings() {
  const settings = {
    ...
    rlMode: rlMode.checked,
    learningMode: learningMode.checked,
...
function restoreSettings() {
  chrome.storage.local.get(STORAGE_KEY).then((result) => {
    ...
    rlMode.checked = s.rlMode === true;
    learningMode.checked = s.learningMode === true;
...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg?.source === 'zorr-bot-content') {
    if (msg.action === 'status') {
      ...
      // Update learning status from RL stats
      if (msg.value.rlStats && demoSpan) {
        demoSpan.textContent = msg.value.rlStats.demos || 0;
      }
      if (msg.value.learningMode && learningStatus) {
        learningStatus.style.display = 'block';
      } else if (learningStatus) {
    ...
    } else if (msg.action === 'rlEpsilon') {
      if (rlEpsilon) rlEpsilon.textContent = msg.value;
    }
  }
});
...
// RL mode toggle
rlMode.addEventListener('change', () => {
  if (rlMode.checked && learningMode.checked) {
    learningMode.checked = false; // mutual exclusion
    learningStatus.style.display = 'none';
    sendToContent('updateSettings', { learningMode: false });
  }
  saveSettings();
  sendToContent('updateSettings', { rlMode: rlMode.checked });
});

// Learning mode toggle
learningMode.addEventListener('change', () => {
  if (learningMode.checked && rlMode.checked) {
    rlMode.checked = false; // mutual exclusion
    sendToContent('updateSettings', { rlMode: false });
  }
  ...
  saveSettings();
  sendToContent('updateSettings', { learningMode: learningMode.checked });
});
