...
(function () {
  ...
  const CFG = {
    ...
    learningMode: false,          // observe + learn from user actions
  };
  ...
  async function tick() {
    ...
    try {
      ...
      // ── 3.0: Learning mode - observe user, don't control ──
      if (CFG.learningMode && RL) {
        // Just observe and record user actions, do NOT control
        RL.recordUserAction(world);
        return;
      }
  ...
  window.addEventListener('message', e => {
    ...
    else if (action === 'updateSettings' && value) {
      // Handle learning mode toggle
      if (value.learningMode !== undefined && RL) {
        if (value.learningMode && !CFG.learningMode) {
          RL.startLearning();
        } else if (!value.learningMode && CFG.learningMode) {
          RL.stopLearning();
        }
      }
  ...
  function broadcast(state) {
    // Include RL stats in status
    if (RL) {
      state.rlStats = RL.getStats();
    }
    state.learningMode = CFG.learningMode;
